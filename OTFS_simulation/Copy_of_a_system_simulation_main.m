% OTFS与OFDM在低轨卫星系统下的仿真代码重写
% 重写代码环境基于matlab R2022a


% 注释内容————董开原
% 该文件为文件simulation_main.m的集合版
...
% info_tx_OTFS_pilot_DD = info_tx_OTFS_pilot_DT*Fn_N_OTFS;
info_tx_OTFS_pilot_FT = Fn_M_pilot'*info_tx_OTFS_pilot_DT;
...
% 信道环境生成
    ...
    elseif sum(strcmp(channel_module,{'NTN_TDL','NTN_TDL_Moving','Multipath_delay','Singlpath_doppler','Multipath_doppler'}))
       ...
       channel_delay_taps = [0,1];
       channel_doppler_taps = [5,10];
       channel_amplitude_coef_taps = [0.9,0.15];

channel_doppler_taps = round(channel_doppler_taps/(M/(M+length_CP+length_ZP)))*(M/(M+length_CP+length_ZP)); 
        % 这一步相当于是让多普勒能够落在网格之上，CP和ZP的引入会使得符号周期变长，多普勒的相位影响变大
...
% % 信号同步 抵消掉大多普勒造成的影响，这里进行了简化。如有需要另行编写
 ...
 %           channel_doppler_taps = (channel_doppler_taps - channel_doppler_taps(1));
        channel_doppler_taps = mod(channel_doppler_taps+N/2,N)-N/2;
    end
...
%  info_rx_OTFS_deinterval_FT = Fn_M_pilot*info_rx_OTFS_deinterval_DT;
%  info_rx_OTFS_deinterval_DD = info_rx_OTFS_deinterval_DT*Fn_N_OTFS;
...
if exist('info_tx_OTFS_DT','var')
    if strcmp(channel_estimation_mode_OTFS,'None')
        ...
        [info_rx_OTFS_domain,channel_H_est_OTFS_domain] = ...
            ...
            pilot_Indx_OTFS,SNR(i_SNR),sigma_2(i_SNR), Channel_DT,interval_mode,length_interval);
    end
end
...
%%%%%%%% 方案 1%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% % 方案1中，OTFS符号的频偏估计范围是0到Δf,(不考虑循环前缀),
% 实际由于循环前缀，会略小于估计范围
% 
% 
% exp_index = (exp(1i*2*pi*(((0:M-1)+(31+length_CP+length_ZP)).'*(0:N-1)/(N*(M+length_CP+length_ZP)))));
% channel_H_est_OTFS_domain = channel_H_est_OTFS_domain./exp_index;% 去掉e^(j2pai * vlT/M)对估计信号的影响
% 
% 
% channel_H_est_OTFS_domain2 = zeros(size(channel_H_est_OTFS_domain));
% channel_H_est_OTFS_domain2(abs(channel_H_est_OTFS_domain)>=abs(max(max(channel_H_est_OTFS_domain))))= sqrt(M*N);
% 
% channel_H_est_OTFS_Dt= ifft(channel_H_est_OTFS_domain2*(M+length_interval),(M+length_interval)*N,2);% 对行作(M+CP)N点ifft
% channel_H_est_OTFS_doppler = channel_H_est_OTFS_Dt(1,length_interval+1:M+length_interval).';% 截取首径，生成e^(j2pai * vlT/M),符号（.'是非共轭转置）
% info_rx_OTFS_domain = info_rx_OTFS_domain./channel_H_est_OTFS_doppler;

%%%%%%%% 方案 2%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% % 方案2针对负频偏做调整，现在OTFS符号的频偏估计范围是正负(Δf/2),(不考虑循环前缀)
% 实际由于循环前缀，会略小于估计范围
% 缺点：在有分数多普勒时，去除信道估计影响的部分会严重失真，脉冲会弥散在网格中，
% 当不存在分数多普勒时，此选项最优 


% exp_index = (exp(1i*2*pi*(((0:M-1)+(31+length_CP+length_ZP)).'*(-N/2:N/2-1)/(N*(M+length_CP+length_ZP)))));
% exp_index = [ exp_index(:,N/2+1:N) exp_index(:,1:N/2)];
% channel_H_est_OTFS_domain = channel_H_est_OTFS_domain./exp_index;% 去掉e^(j2pai * vlT/M)对估计信号的影响
% 
% channel_H_est_OTFS_domain2 = zeros(size(channel_H_est_OTFS_domain));
% channel_H_est_OTFS_domain2(abs(channel_H_est_OTFS_domain)>=abs(max(max(channel_H_est_OTFS_domain))))= N;
% channel_H_est_OTFS_domain2= [channel_H_est_OTFS_domain2(:,N/2+1:N),channel_H_est_OTFS_domain2(:,1:N/2) ];
% channel_H_est_OTFS_Dt= ifft(channel_H_est_OTFS_domain2*(M+length_CP+length_ZP),(M+length_CP+length_ZP)*N,2);% 对行作(M+CP)N点ifft
% 
% exp_index2 = exp(-1i*pi*(length_CP+length_ZP:M+length_CP+length_ZP-1).'/(M+length_CP+length_ZP)); % 针对负频偏做的调整
% channel_H_est_OTFS_doppler = channel_H_est_OTFS_Dt(1,length_CP+length_ZP+1:M+length_CP+length_ZP).'.*exp_index2;% 截取首径，生成e^(j2pai * vlT/M),符号（.'是非共轭转置）
% 
% info_rx_OTFS_domain = info_rx_OTFS_domain./channel_H_est_OTFS_doppler; % 去掉e^(j2pai * vlT/M)对数据信号的影响

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% %%%%%%% 方案 3%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% % 基于方案2，做的优化，考虑分数多普勒的情况。
% % 对整个信道估计矩阵，去除主径多普勒的影响

[index_m, index_n] = find(abs(channel_H_est_OTFS_domain) ==max(max(abs(channel_H_est_OTFS_domain) )));
exp_index = (exp(1i*2*pi*(((0:M-1)+(31+length_CP+length_ZP)).'*(index_n-1)/(N*(M+length_CP+length_ZP)))));
channel_H_est_OTFS_domain = channel_H_est_OTFS_domain./exp_index;% 去掉e^(j2pai * vlT/M)对估计信号的影响

channel_H_est_OTFS_domain2 = zeros(size(channel_H_est_OTFS_domain));
channel_H_est_OTFS_domain2(abs(channel_H_est_OTFS_domain)>=abs(max(max(channel_H_est_OTFS_domain))))= 1;
channel_H_est_OTFS_domain2= [channel_H_est_OTFS_domain2(:,N/2+1:N),channel_H_est_OTFS_domain2(:,1:N/2) ];
channel_H_est_OTFS_Dt= ifft(channel_H_est_OTFS_domain2*N*(M+length_CP+length_ZP),(M+length_CP+length_ZP)*N,2);% 对行作(M+CP)N点ifft
...
% %%%%%%% 方案4%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% 废案，无效
% channel_H_est_OTFS_FT = zeros(size(channel_H_est_OTFS_domain));
% channel_H_est_OTFS_DDtemp = zeros(size(channel_H_est_OTFS_domain));
% for m=1:M
%  for n =1:N
%         channel_H_est_OTFS_DDtemp(m,n) = channel_H_est_OTFS_domain(m,n);
%         channel_H_est_OTFS_FTtemp = Fn_M_pilot*channel_H_est_OTFS_DDtemp*Fn_N_pilot';
%         channel_H_est_OTFSdoppler = sum(exp(1i*n*(1:M)/(N+M+length_CP+length_ZP)))/M;
%         channel_H_est_OTFS_FT = channel_H_est_OTFS_FT+channel_H_est_OTFS_FTtemp*channel_H_est_OTFSdoppler;
% 
%         channel_H_est_OTFS_DDtemp(:,:) = 0;
%  end
% end
% channel_H_est_OTFS_domain = Fn_M_pilot'*channel_H_est_OTFS_FT*Fn_N_pilot;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% channel_H_est_OTFS_FT = Fn_M_pilot*channel_H_est_OTFS_domain*Fn_N_pilot';
...
if exist('info_tx_OFDM_DT','var')
    if strcmp(channel_estimation_mode_OFDM,'None')
        ...
        [info_rx_OFDM_domain,channel_H_est_OFDM_domain] = ...
            ...
            pilot_Indx_OFDM,SNR(i_SNR),sigma_2(i_SNR), Channel_DT,interval_mode,length_interval);
...
% 数字解调（OTFS/OFDM）,输出尺寸（M，N_data）
    ...
    if exist('info_tx_OTFS_DT','var') 
        ...
        % info_rx_deOTFS_DD = Fn_M_OTFS * info_rx_OTFS_equalization_DT;
    end
    if exist('info_tx_OFDM_DT','var')
...
%     end
%     legend(legend_str2)
%     axis([SNR_dB(1) SNR_dB(i_SNR) 1e-3 1e-0])
%     grid on
%     xlabel('SNR(dB)')
%     ylabel('BER')
%     title([channel_module,'环境下OTFS与OFDM的误码率以及SNR之间的关系'],'Interpreter','none')
